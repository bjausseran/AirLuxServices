...
export class RoomController extends Controller
{
  ...
  updateStatement: string = `UPDATE ${this.tableName} SET name = ?, building_id = ? WHERE id = ?`;
  insertStatement: string = `INSERT INTO ${this.tableName} (id, name, building_id) VALUES (?, ?, ?)`;
